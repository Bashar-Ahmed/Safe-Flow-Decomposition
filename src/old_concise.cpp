...
Old_Concise::Old_Concise(const std::string &graph) : Graph(graph)
{
    f_in.resize(nodes, 0);
...
void Old_Concise::decompose_path()
{
...
void Old_Concise::insert(std::shared_ptr<AC_Trie> &root, std::vector<int> &str)
{
    ...
    if (root->children.empty())
    {
        if (root->is_fail)
            return;
        old_concise_repr.emplace_back(root->flow, str);
    }
    else
    {
...
void Old_Concise::compress_path(double flow, std::deque<int> &route, std::shared_ptr<AC_Trie> &root)
{
    trace(route, flow);
    if (concise_repr.empty())
    {
        concise_repr.push_back({route, {cut(route.front(), route.back(), flow)}});
        return;
    }
    auto &g = concise_repr.back();
    trace(g, route);
    auto it = route.begin();
    for (auto &&path_element : g.first)
    {
        if (it == route.end())
            return;
        if (*it == path_element)
    ...
    {
        trace(*it);
        g.first.push_back(*it);
        it++;
    }
    g.second.push_back(cut(route.front(), route.back(), flow));
...
void Old_Concise::print_maximal_safe_paths()
{
    // std::cout << metadata << "\n\n";
    // for (auto &&path : old_concise_repr)
    // {
    // 	std::cout << path.first << " ";
